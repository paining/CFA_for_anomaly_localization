    ...
    def extract_endpoints(self, inputs):
        ...
        for idx, block in enumerate(self._blocks):
            ...
            if x.size(1) == 40:
                ...
                feature_16x.append(x)

        x = self._swish(self._bn1(self._conv_head(x)))
        endpoints["reduction_{}".format(2)] = torch.cat(feature_4x[-5:], dim=1)
        endpoints["reduction_{}".format(3)] = torch.cat(feature_8x[-3:], dim=1)
        endpoints["reduction_{}".format(4)] = torch.cat(feature_16x[-1:], dim=1)
